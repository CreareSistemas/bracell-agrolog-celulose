package br.com.crearesistemas.pcsi.wsrecebegruassiteindustrial.dto;
...
public class RecebeGruasSiteIndustrialResponse {
	...
	private Boolean addGruaPatio(Config config, GruaEntity grua) {
		...
		if (grua.getName().contains("31003")) {
			gruaMovel.setIdPileCode(16);
			gruaMovel.setPileName("C016");
		} else {
			gruaMovel.setIdPileCode(13);
			gruaMovel.setPileName("C014");	
		}
	...
	private Boolean addGruaMesa(Config config,GruaEntity grua) {
		...
		if (config.isLocalMesas(grua.getIdLocal())  && !config.configCraneFix()) {
			...
			gruaFixa.setIdLocal(config.getLocalIdMesaByCrane(grua.getName()));
			grua.setIdLocal(config.getLocalIdMesaByCrane(grua.getName()));
		}
