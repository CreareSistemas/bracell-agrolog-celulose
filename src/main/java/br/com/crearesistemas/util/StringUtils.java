...
public final class StringUtils {
	...
	public static String joinString(List<?> list) {
		StringBuilder sb = new StringBuilder();
		if (list != null) {
			int size = list.size();
			if (size > 0) {
				sb.append(SINGLE_QUOTE).append(list.get(0)).append(SINGLE_QUOTE);
			}
			for (int i = 1; i < size; i++) {
				sb.append(COMMA).append(SINGLE_QUOTE).append(list.get(i)).append(SINGLE_QUOTE);
			}
		}
		return sb.toString();
	}
